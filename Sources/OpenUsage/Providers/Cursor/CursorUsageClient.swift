...
struct CursorUsageClient: Sendable {
    ...
    /// GET the dashboard's usage-summary endpoint — the only REST call that returns full usage for
    /// enterprise/team (`TOKEN_BASED_CONTRACT`) accounts. Uses the same session cookie as the other
    /// REST calls. Returns nil when the access token carries no usable session.
